...
def get_info(t):
    ...
    end_time = dcc[e]["end"]
    check_in = re.compile(ur"检票口\d*\w").search(t).group(0)[3:]
    return start_station_name, to_station_name, cost_hour, cost_mins, cc, zc, day, month, start_time, end_time, check_in


def make_ics(t):
    start_station_name, to_station_name, cost_hour, cost_mins, cc, zc, day, month, start_time, end_time, check_in = get_info(
        t)

    cal = Calendar()
    event = Event()
    event.add('dtstart',
              datetime(2017, int(month), int(day), int(start_time.split(":")[0]), int(start_time.split(":")[1]), 0,
                       tzinfo=pytz.timezone("Asia/Shanghai")))
    event.add('dtend', datetime(2017, int(month), int(day), int(end_time.split(":")[0]), int(end_time.split(":")[1]), 0,
                                tzinfo=pytz.timezone("Asia/Shanghai")))
